...
namespace Task29_2
{
    public partial class Form1 : Form
    {
        ...
        {
            Taryan taryan = new Taryan();
            foreach (String point in nodePositions.Keys) {
                taryan.AddVertex(Convert.ToInt32(point));
            }
            foreach (Tuple<string,string> tuple in edges)
            {
                taryan.AddEdge(Convert.ToInt32(tuple.Item1),Convert.ToInt32(tuple.Item2));
            }
            List<int> list = taryan.TopologicalSort();
            edges.Clear();
            for(int i=1;i<list.Count;i++) {
                edges.Add(new Tuple<string,string> (list[i - 1].ToString(), list[i].ToString()));
            }               
            this.Invalidate();
        }
    }
