...
namespace MyHashSetLib
{
    ...
    internal class MyHashSet<T> where T : IComparable<T>
    {
        ...
        private int Compare(T key1,T key2)
        {
            ...
            var words1 = str1.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)
                         .Select(word => word.Length)
                         .OrderBy(length => length);

            var words2 = str2.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)
                             .Select(word => word.Length)
                             .OrderBy(length => length);

            int minLength = Math.Min(words1.Count(), words2.Count());

            for (int i = 0; i < minLength; i++)
            {
                if (words1.ElementAt(i) < words2.ElementAt(i))
                    return -1;
                if (words1.ElementAt(i) > words2.ElementAt(i))
                    return 1;
            }
        ...
        public void add(T key, object value=null)
        {
            ...
            for (int i = 0; i < table.Length; i++)
            {
                if (table[i] != null)
                {
                    ...
                    while (true)
                    {
                        ...
                        else if (Compare(min,node.Key)>0) min= node.Key;
                        if (node.Next == null) break;
                        node = node.Next;
                    }
                }
            ...
            for (int i = 0; i < table.Length; i++)
            {
                if (table[i] != null)
                {
                    ...
                    while (true)
                    {
                        ...
                        else if (Compare(max,node.Key) < 0) max = node.Key;
                        if (node.Next == null) break;
                        node = node.Next;
                    }
                }
        ...
        public List<T> subSet(T fromElement, T toElement)
        {
            ...
            for (int i = 0; i < table.Length; i++)
            {
                if (table[i] != null)
                {
                    ...
                    while (true)
                    {
                        if (Compare(node.Key, fromElement) >=0 && Compare(node.Key, toElement) <0) list.Add(node.Key);
                        if (node.Next == null) break;
                        node = node.Next;
                    }
                }
        ...
        public List<T> headSet(T toElement)
        {
            ...
            for (int i = 0; i < table.Length; i++)
            {
                if (table[i] != null)
                {
                    ...
                    while (true)
                    {
                        if (Compare(node.Key, toElement) < 0) list.Add(node.Key);
                        if (node.Next == null) break;
                        node = node.Next;
                    }
                }
        ...
        public List<T> tailSet(T fromElement, T toElement)
        {
            ...
            for (int i = 0; i < table.Length; i++)
            {
                if (table[i] != null)
                {
                    ...
                    while (true)
                    {
                        if (Compare(node.Key, fromElement) >= 0) list.Add(node.Key);
                        if (node.Next == null) break;
                        node = node.Next;
                    }
                }
